...
def mainCommands(gender=ResponsiveVoice.MALE, rate=0.52, pitch=0.50, vol=1):
    ...
    def response(voice):
        ...
        if "8 mart" in voice:
            ...
            exit()
        def mail():
            engine.say("tanımlı olan alıcı adresine ne göndermek istiyorsun", gender=gender, rate=rate, pitch=pitch, vol=vol)
            content = record()
            mail = smtplib.SMTP("smtp.gmail.com", 587)
            mail.ehlo()
            mail.starttls()
            mail.login("alierentabak0156@gmail.com", "Eren_6627")
            mail.sendmail("alierentabak0156@gmail.com", "aeren7630@gmail.com", content)

        if "mail gönder" in voice:
            mail()
            engine.say("mailini gönderdim", gender=gender, rate=rate, pitch=pitch, vol=vol)
